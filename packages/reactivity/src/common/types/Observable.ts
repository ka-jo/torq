...
/**
 * @privateRemarks
 * Torq uses the Observable as the core primitive for its reactive philosophy internally.
 ...
 *
 * Torq's philosophy is that the chief concern of reactivity is the ability to subscribe
 * to data — not necessarily the means of retrieving and updating it.
 *
 * Torq's Observable is meant to mirror the TC39 proposal and the larger Observable
 * ecosystem as a whole, but due to the fact that Refs are the only observable type in
 * Torq and are a much more focused use case, there are some notable divergences:
 * - Torq has no concept of "unicast" observables; Refs are multicast by design
 * - Torq has no concept of "hot" or "cold" observables; Refs are hot by design
 ...
 * - All Observables in Torq produce values and some Observables must be computed to get the
 *   most recent value
 * - Observers subscribe to *future* values, not current values
 *
 * This Observable type is meant to model how Observables are used internally in Torq,
